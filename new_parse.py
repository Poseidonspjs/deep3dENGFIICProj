...
class Stereo(object):
    ...
    def sgbm_create(self, minDisparity, numDisparities, blockSize, mode=cv2.STEREO_SGBM_MODE_HH):
...
def flow(prev_frame, cur_frame, tvl1=None):
    if tvl1 is None:
        tvl1 = cv2.optflow.DualTVL1OpticalFlow_create() if hasattr(cv2, 'optflow') else cv2.createOptFlow_DualTVL1()
...
def make_flow_data(input_path, output_path):
    ...
    while True:
        ...
        if pframe is None:
            pframe = frame
        f = flow(pframe, frame)
        f = np.clip((f + 40) * 255.0 / 80.0, 0.0, 255.0).astype(np.uint8)
